...
def img2label_paths(img_paths):
    ...
    # x.rsplit(sa, 1) 将图片路径从sa处分隔开，分开的结果不包含sa
    # str.join是将列表中的元素以str连接起来，表现形式为list[0]+str+list[1]+...
    # .rsplit('.', 1)[0] 取xxx.jpg中的xxx
    # +'.txt'
    ...
    def __init__(self,
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    f += glob.glob(str(p / '**' / '*.*'), recursive=True) # 获得当前文件夹下及其子文件夹下的所有文件
