...
from models.yolo_pruned import ModelPruned as Model
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        bn_mask_dict = ckpt['model'].maskbndict
        model = Model(bn_mask_dict, ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'runs/val/exp10/pruned_model.pt',
    ...
    parser.add_argument('--evolve', type=int, nargs='?', const=200, help='evolve hyperparameters for x generations')
    ...
    # 修改workers，如果是windows+pycharm，需要将default设置为0
