...
from utils.general import (LOGGER, check_dataset, check_file, check_img_size,
                           ...
                           init_seeds, intersect_dicts, labels_to_class_weights, methods,
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    amp = False  # 这里的混合精度训练是否对剪枝造成影响，还有待验证。
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # # Backward, scaler的作用主要是避免使用fp16进行运算时的梯度下溢。
            # scaler.scale(loss).backward()

            # 但是在稀疏训练的时候使用上述功能梯度不准确会导致下面正则化出现问题
            loss.backward()

            # -----------------  sparsity training --------------------------#
            srtmp = opt.sr * (1 - 0.9 * epoch / epochs)  # 刚开始稀疏因子为sr，到训练后期逐步减小
            if opt.st:
                ignore_bn_list = []
                for k, m in model.named_modules():
                    if isinstance(m, Bottleneck):
                        # 如果Bottleneck结构中存在shortcut操作，那么就跳过对这个结构的剪枝，以及Bottleneck之前的一个卷积的剪枝
                        # 因为如果对那个卷积剪枝，Bottleneck的输入就会发生变化，也会造成add时维度不相等的情况出现
                        if m.add:
                ...
                for k, m in model.named_modules():
                    if isinstance(m, nn.BatchNorm2d) and (k not in ignore_bn_list):
                        ...
                        # 同时为了避免BN层weight为0时，bias不为0时，移除BN层及其对应的卷积，bias会对模型性能产生影响
                        # 所以对bias也加入较大的正则化，这里选择10倍的sr
            ...
            # Log，打印显存，进行的轮次，损失，target的数量和图片的size等信息
            if RANK in {-1, 0}:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--sr', type=float, default=0.0005, help='L1 normal sparse rate')

    parser.add_argument('--weights', type=str, default=ROOT / 'weights/yolov5n_face.pt', help='initial weights path')
